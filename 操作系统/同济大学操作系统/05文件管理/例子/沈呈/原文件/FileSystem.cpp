...
bool FileSystem::createFolder(string folderName)
{
	...
	{
		cout << "此路经中有重名的文件,不能执行此操作!  如需查看基本操作,请输入?";
		return false;
	}
	if(enoughBlock(blockForFCB,blockForContent) == false)
	{	
		cout << "空间不足,不能执行此操作!  如需查看基本操作,请输入?";
		return false;
	}
		...
		if(path.find("\\") > path.length())
		{
			...
			for(int i = 1; i < this->fatTable.fatBlockList[blockIndex].getLengthInUse() / 64; i++)
			{
				...
				if(fileRecord.name == path)
				{
					...
					{
						cout << path + "是文件,不能执行此操作!  如需查看基本操作,请输入?";
						return false;
					}
				}
			}
			...
			cout << "当前目录下没有文件夹" + path + ",  不能执行此操作!  如需查看基本操作,请输入?";
			return false;
		}
		else
		{
			...
			for(int i = 1; i < this->fatTable.fatBlockList[blockIndex].getLengthInUse() / 64; i++)
			{
				...
				if(fileRecord.name == s)
				{
					...
					if(aFCB.folderOrFile == true)
					{
						...
						{
							cout << "路径 " + path + "错误,不能执行此操作!  如需查看基本操作,请输入?";
							return false;
						}
					}
					else
...
bool FileSystem::del(string fileName)
{
	...
	for(int i = 1; i < this->fatTable.fatBlockList[blockIndex].getLengthInUse() / 64; i++)
	{
		...
		if(fileRecord.name == fileName)
		{
			...
			{
				cout << fileName + "是文件夹，不能进行此操作!  如需查看基本操作,请输入?";
				return false;
			}
		}
	}
	...
	cout << "当前目录下不存在文件 " + fileName + " ，不能进行此操作!  如需查看基本操作,请输入?";
	return false;
}
...
bool FileSystem::deltree(string folderName)
{
	...
	for(int i = 1; i < this->fatTable.fatBlockList[blockIndex].getLengthInUse() / 64; i++)
	{
		...
		if(fileRecord.name == folderName)
		{
			...
			{
				cout << folderName + "是文件，不能进行此操作!  如需查看基本操作,请输入?";
				return false;
			}
		}
	}
	...
	cout << "当前目录下不存在文件夹 " + folderName + " ，不能进行此操作!  如需查看基本操作,请输入?";
	return false;
}
...
void FileSystem::instruction()
{
	cout << "操作说明：" << endl;
	cout << "    dir                      --显示当前文件夹内的信息目录" << endl;
	cout << "    cd..                     --进入上级目录" << endl;
	cout << "    cd 路径名                --进入路径名所对应的目录" << endl;
	cout << "    md 文件夹名              --在本目录下创建以文件夹名命名的文件夹" << endl;
	cout << "    mf 文件名                --在并目录下创建以文件名命名的文件" << endl;
	cout << "    deltree 文件夹名         --删除文件夹并将其中的文件夹和文件一并删除" << endl;
